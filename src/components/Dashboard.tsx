...
function Avatar({ name, size = 'md' }: { name: string; size?: 'sm' | 'md' }) {
  ...
  return (
    <div className={cn(sizeClass, color, 'rounded-full flex items-center justify-center text-white font-medium shrink-0')}>
...
function BugDetailModal({ bug, onClose }: { bug: Bug; onClose: () => void }) {
  ...
  const handleAddComment = (e: React.FormEvent) => {
    ...
    addComment(bug.id, { bugId: bug.id, userId: user.id, username: user.username, text: commentText.trim() });
    setCommentText('');
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-slate-800 border border-slate-700/50 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            ...
            <div className="bg-slate-700/30 rounded-lg p-3">
              ...
              <p className="text-sm text-white">{new Date(bug.createdAt).toLocaleDateString()}</p>
            </div>
            <div className="bg-slate-700/30 rounded-lg p-3">
              <p className="text-xs text-slate-400 mb-1">Updated</p>
              <p className="text-sm text-white">{new Date(bug.updatedAt).toLocaleDateString()}</p>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-4">
          ...
          <StatCard label="Open" value={stats.open} icon={<AlertCircle className="w-4 h-4 text-blue-400" />} color="bg-blue-500/20" />
          <StatCard label="In Progress" value={stats.inProgress} icon={<Clock className="w-4 h-4 text-amber-400" />} color="bg-amber-500/20" />
          <StatCard label="Resolved" value={stats.resolved} icon={<CheckCircle className="w-4 h-4 text-emerald-400" />} color="bg-emerald-500/20" />
          <StatCard label="Closed" value={stats.closed} icon={<XCircle className="w-4 h-4 text-slate-400" />} color="bg-slate-500/20" />
          <StatCard label="Critical" value={stats.critical} icon={<Zap className="w-4 h-4 text-red-400" />} color="bg-red-500/20" />
        </div>
        ...
        <div className="space-y-3">
          {filteredBugs.length === 0 ? (
            <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-slate-300 mb-2">No bugs found</h3>
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className={cn(
                      ...
                      bug.priority === 'critical' ? 'bg-red-400 animate-pulse' :
                      bug.priority === 'high' ? 'bg-orange-400' :
                      bug.priority === 'medium' ? 'bg-yellow-400' : 'bg-emerald-400'
                    )} />
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-slate-400 line-clamp-1 mb-2">{bug.description}</p>
